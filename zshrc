...
alias build='gcc -ggdb -std=c17 -O0 -pedantic -Wall -Wextra'
...
alias gp='git push'
alias gpu='git pull'
alias gpurs='git pull --recurse-submodules'
alias gs='git status'
alias javac='javac -Xlint:all'
alias la='ls -lAhv'
alias ll='ls -lhv'
alias l='la'
alias manl="MANPATH=$HOME/.local/share/man man"
alias psv='ps -eHo pid,pgid,sid,ppid,tty,user,ruser,time,stat,command'

build++ () {
	local compiler=$1
	shift
	[[ $compiler != g++ && $compiler != clang++ ]] && { echo weird; return; }
	"$compiler" -std=c++20 -O0 -pedantic -Wall -Wextra "$@"
}
